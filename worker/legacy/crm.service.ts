...
/**
 * Detect lead origin based on sender format
 * LID format (15+ digits) = Facebook/Instagram Lead Ads
 * Regular phone = WhatsApp direto
 */
export function detectLeadOrigin(sender: string): string {
  // Clean the sender ID
  ...
  // LID format from Facebook/Instagram Lead Ads typically has 15+ digits
  // Regular Brazilian phones have 12-13 digits (55 + DDD + 8-9 digits)
  if (cleanSender.length >= 15) {
    console.log(`[CRM] 📱 Origem detectada: facebook_ads (LID: ${cleanSender.substring(0, 10)}...)`);
    return 'facebook_ads';
  }
  
  // Check if sender originally had @lid suffix
  if (sender.includes('@lid')) {
    console.log(`[CRM] 📱 Origem detectada: facebook_ads (@lid suffix)`);
    return 'facebook_ads';
  }
  
  console.log(`[CRM] 📱 Origem detectada: whatsapp_direto`);
    ...
    if (existing) {
      ...
      const newLead = await db.createLead({
          ...
          metadata: {
              status: "novo",
              total_mensagens: 1,
              origem: detectLeadOrigin(telefone),
              canal: "WhatsApp",
