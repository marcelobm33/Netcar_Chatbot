...
import {
  ...
  SHOW_OPTIONS_PATTERNS,
  LOW_SIGNAL_RESPONSES,
  INFO_STORE_KEYWORDS,
  OUT_OF_SCOPE_KEYWORDS,
} from '../config/router-keywords';
...
export function extractTradeIn(message: string): { hasTradeIn: boolean; model?: string } | null {
  ...
  for (const pattern of tradeInPatterns) {
    if (normalized.includes(pattern)) {
      // Try to extract model mentioned
...
export function routeMessage(
  ...
  const extractedModel = extractCarModel(userMessage);  // BUG-001 FIX
