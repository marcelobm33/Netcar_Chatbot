 ...
 */
export function buildDynamicHoursResponse(hours: StoreHoursConfig): string {
  ...
  // Resposta conversacional baseada no dia atual
  if (dayOfWeek === 0) {
    // Domingo
...
export async function checkFAQScriptAsync(message: string, env: any): Promise<ScriptResponse> {
  ...
  // HORÁRIO - busca dinâmico do DB
  for (const pattern of HOURS_PATTERNS) {
    if (pattern.test(lowerMsg)) {
      console.log('[SCRIPT] FAQ matched: hours (dynamic)');
      
      // Busca horários da config
      ...
      try {
        ...
        console.error('[SCRIPT] Failed to fetch store_hours, using defaults:', e);
      }
      
      return {
        matched: true,
        response: buildDynamicHoursResponse(hoursConfig),
        action: 'respond',
